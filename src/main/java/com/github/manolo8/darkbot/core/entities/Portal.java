...
public class Portal extends Entity {
    ...
    public Portal(int id, int searchType, int searchX, int searchY, Map target, int factionId) {
        super(id);
        this.matcher = new PortalMatcher(searchType, searchX, searchY, target != null && target.id == -3);
    ...
    private static class PortalMatcher {
        ...
        private final boolean alwaysCompare;

        PortalMatcher(int searchType, int searchX, int searchY, boolean alwaysCompare) {
            this.searchType = searchType;
            this.searchX    = searchX;
            this.searchY    = searchY;
            this.alwaysCompare = alwaysCompare;
        }
        ...
        boolean matches(int x, int y, int type) {
            return ((searchType != -1 && searchType == type) || // By type
                    ...
                    (!alwaysCompare || x == searchX && y == searchY); // Avoid centered
        }
    }
}
