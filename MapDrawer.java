...
public class MapDrawer extends JPanel {
    ...
    private void drawHealth(Graphics2D g2) {
        ...
        if(!hero.pet.removed && (hasFlag(DisplayFlag.SHOW_PET))) {
            drawString(g2, hero.pet.playerInfo.username, 10 + (mid - 200) / 2, height - 100, Align.MID);
            drawHealth(g2, hero.pet.health, 10, height - 95, mid - 200, 10);
            fuel = petManager.getPetStats(PetStatsType.FUEL);
            if (fuel != null) {
                currentFuel = fuel.getCurr();
                maxFuel = fuel.getTotal();
                drawPetFuel(g2, 10, height - 75, mid - 200, 10);
            }
        }
    ...
    private void drawPetFuel(Graphics2D g2, int x, int y, int width, int height) {
        ...
        boolean displayAmount = height >= 8;

        g2.setColor(cs.NANO_HULL.darker());
        g2.fillRect(x, y, width, height);

        g2.setColor(cs.NANO_HULL);
        g2.fillRect(x, y + height + width, (int) (maxFuel == 0 ? 1 : ((double) currentFuel / (double) maxFuel) * width), height);

        g2.setColor(cs.TEXT);
        if (displayAmount)
            drawString(g2, HEALTH_FORMAT.format(currentFuel) + "/" +
                    HEALTH_FORMAT.format(maxFuel), x + width / 2, y + height - 2, Align.MID);
    }
